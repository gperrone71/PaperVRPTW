...
public class GenerateDataSet {
	...
	public void GenerateDS(String strPath, String strFileSuff, BatchConfig configParam) {
		...
		switch (configParam.getcTaskDistribution()) {
			...
			case 'R': {
				...
				listTasks = GenerateClusterTasksList(iNumClusteredTasks, 0, configParam);
	...
	private List<Task> GenerateClusterTasksList(int nTasks, int nStartingNodeID, BatchConfig configParam) {
		...
		for (int i = 0; i < iNumClusters; i++) {
			...
			// if the type of cluster is a Gaussian then generate the number of tasks for the cluster
			if (configParam.getcClusterType() == 'G') 
				do 
					iNumNodesPerCluster = (int) (rndGauss.nextGaussian()*(nTasks/(2*iNumClusters)) + (nTasks/iNumClusters));		// gaussian distr w/ avg = n/c and sigma = n/2*c
				while (iNumNodesPerCluster <= 0);				// discard all values of the normal distribution that are < 0
			
				
			// adjust the number of nodes for the last cluster 
			if ( (i + 1) == iNumClusters) 			// if I am creating the last cluster
				iNumNodesPerCluster = nTasks - listTasks.size();		// then the number of tasks for the last cluster is given by what is missing to reach the total number of tasks

			if (iNumNodesPerCluster < 0)
				iNumNodesPerCluster = 0;
			
			PerroUtils.print(" +--- Cluster " + i + " has " + iNumNodesPerCluster + " nodes.", true);
